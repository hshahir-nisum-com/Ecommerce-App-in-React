...
userSchema.pre("findOneAndUpdate", async function (next) {
  console.log("this ::::::::::", this._update.newpassword);
  const salt = await bcrypt.genSalt();
  this._update.newpassword = await bcrypt.hash(this._update.newpassword, salt);
  next();
});
...
userSchema.statics.forgetPass = async function (
  ...
  if (userFound) {
    ...
    if (auth) {
      const userUpdate = await this.findOneAndUpdate(
        { email },
        { newpassword },
        { new: true }
      );
